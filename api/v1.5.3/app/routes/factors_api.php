...
SQLS: 	 [tabla]_count
--*/
$app->get('/factors/count', function () use($app) {

	try{
			$authorized = checkPerm('GET:/factors/count', $app);
			if($authorized){
				$resultText = checkToken($app);
				if(contains("validtoken", $resultText) ){
		      $sqlCode = 'factors_count';
		      ...
		      simpleReturn($app, $sqlCode, $forXSL);
				...
				$connection = null;
				$app->response->body("/factors/count " . ACCESSERROR);
...
SQLS: 	 [tabla]_all
--*/
$app->get("/factors", function() use($app)
{
 	try{
			$authorized = checkPerm('GET:/factors', $app);
			if($authorized){
					$resultText = checkToken($app);
					if(contains("validtoken", $resultText) ){
						$sqlCode = ($app->request()->params('sqlCode') == "" ? 'factors_all' : $app->request()->params('sqlCode') );
			      $forXSL = '../../xsl/count.xsl';
			      simpleReturn($app, $sqlCode, $forXSL);
					} else {
						$connection = null;
					...
					$connection = null;
					$app->response->body("/factors " . ACCESSERROR);
		}
	}
	catch(PDOException $e)
	{
...
--*/
$app->get("/factors/:id", function($id) use($app)
{
 	try{
		$authorized = checkPerm('GET:/factors/:id', $app);
		if($authorized){
				$resultText = checkToken($app);
				if(contains("validtoken", $resultText) ){
					$sqlCode = 'factors_all';
			...
			$connection = null;
			$app->response->body("/factors/:id " . ACCESSERROR);
...
SQLS: 	 [tabla]_add
--*/
$app->post('/factors', function () use($app) {

	try{
		$authorized = checkPerm('POST:/factors', $app);
		if($authorized){
					$resultText = checkToken($app);
					if(contains("validtoken", $resultText) ){
/*
...
*/
							...
							$prepParams = array(
										...
										':unidad1'  => $app->request()->params('unidad1'),
										':unidad2'  => $app->request()->params('unidad2'),
										':id'         => $newId,
										':multip'    	=> $app->request()->params('multip'),
										':adicion'    	=> $app->request()->params('adicion'),
		...
		*/
				...
				$connection = null;
				$app->response->body("/factors (POST) " . ACCESSERROR);
...
SQLS: 	 autogenerado
--*/
$app->put('/factors', function () use($app) {

	try{
		$authorized = checkPerm('PUT:/factors', $app);
		if($authorized){
				$resultText = checkToken($app);
				if(contains("validtoken", $resultText) ){
					$tableName = 'factores';
		      ...
		      $connection = null;
				...
				$connection = null;
				$app->response->body("/factors (PUT) " . ACCESSERROR);
...
MÉTODO: POST
REQUERIMIENTOS: TO-DO identificar el req actualización de usuarios
TESTS: api/factors_update_post.sh
...
SQLS: 	 autogenerado
--*/
$app->post('/factors/update', function () use($app) {

	try{
		$authorized = checkPerm('POST:/factors/update', $app);
		if($authorized){
				$resultText = checkToken($app);
				if(contains("validtoken", $resultText) ){
					$tableName = 'factores';
		      ...
		      $connection = null;
				...
				$connection = null;
				$app->response->body("/factors/update (POST) " . ACCESSERROR);
...
SQLS: 	 autogenerado
--*/
$app->post('/factors/add', function () use($app) {

	try{
		$authorized = checkPerm('POST:/factors/add', $app);
		if($authorized){
				$resultText = checkToken($app);
				if(contains("validtoken", $resultText) ){
					$tableName = 'factores';
		      ...
		      $connection = null;
				...
				$connection = null;
				$app->response->body("/factors/add (POST) " . ACCESSERROR);
...
SQLS: 	 autogenerado
--*/
$app->put('/factors/delete', function () use($app) {

	try{
		$authorized = checkPerm('PUT:/factors/delete', $app);
		if($authorized){
					$resultText = checkToken($app);
					if(contains("validtoken", $resultText) ){
						$tableName = 'factores';
				...
				$connection = null;
				$app->response->body("/factors/delete (PUT) " . ACCESSERROR);

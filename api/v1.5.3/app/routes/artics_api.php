...
SQLS: 	 [tabla]_count
--*/
$app->get('/artics/count', function () use($app) {

	try{
			$authorized = checkPerm('GET:/artics/count', $app);
			if($authorized){
				$resultText = checkToken($app);
				if(contains("validtoken", $resultText) ){
		      $sqlCode = 'artics_count';
		      ...
		      simpleReturn($app, $sqlCode, $forXSL);
				...
				$connection = null;
				$app->response->body("/artics/count " . ACCESSERROR);
...
SQLS: 	 [tabla]_all
--*/
$app->get("/artics", function() use($app)
{
 	try{
			$authorized = checkPerm('GET:/artics', $app);
			if($authorized){
					$resultText = checkToken($app);
					if(contains("validtoken", $resultText) ){
						$sqlCode = ($app->request()->params('sqlCode') == "" ? 'artics_all' : $app->request()->params('sqlCode') );
			      $forXSL = '../../xsl/count.xsl';
			      simpleReturn($app, $sqlCode, $forXSL);
					} else {
						$connection = null;
					...
					$connection = null;
					$app->response->body("/artics " . ACCESSERROR);
		}
	}
	catch(PDOException $e)
	{
...
--*/
$app->get("/artics/:id", function($id) use($app)
{
 	try{
		$authorized = checkPerm('GET:/artics/:id', $app);
		if($authorized){
				$resultText = checkToken($app);
				if(contains("validtoken", $resultText) ){
					$sqlCode = 'artics_all';
			...
			$connection = null;
			$app->response->body("/artics/:id " . ACCESSERROR);
...
SQLS: 	 [tabla]_add
--*/
$app->post('/artics', function () use($app) {

	try{
		$authorized = checkPerm('POST:/artics', $app);
		if($authorized){
					$resultText = checkToken($app);
					if(contains("validtoken", $resultText) ){
/*
...
*/
							...
							$prepParams = array(
										...
										':id'         => $newId,
										':categoria'  => $app->request()->params('categoria'),
										':codigo'     => $app->request()->params('codigo'),
										':nombre'    	=> $app->request()->params('nombre'),
										':vencim'    	=> $app->request()->params('vencim'),
		...
		*/
				...
				$connection = null;
				$app->response->body("/artics (POST) " . ACCESSERROR);
...
SQLS: 	 autogenerado
--*/
$app->put('/artics', function () use($app) {

	try{
		$authorized = checkPerm('PUT:/artics', $app);
		if($authorized){
				$resultText = checkToken($app);
				if(contains("validtoken", $resultText) ){
					$tableName = 'articulos';
		      ...
		      $connection = null;
				...
				$connection = null;
				$app->response->body("/artics (PUT) " . ACCESSERROR);
...
MÉTODO: POST
REQUERIMIENTOS: TO-DO identificar el req actualización de usuarios
TESTS: api/artics_update_post.sh
...
SQLS: 	 autogenerado
--*/
$app->post('/artics/update', function () use($app) {

	try{
		$authorized = checkPerm('POST:/artics/update', $app);
		if($authorized){
				$resultText = checkToken($app);
				if(contains("validtoken", $resultText) ){
					$tableName = 'articulos';
		      ...
		      $connection = null;
				...
				$connection = null;
				$app->response->body("/artics/update (POST) " . ACCESSERROR);
...
SQLS: 	 autogenerado
--*/
$app->post('/artics/add', function () use($app) {

	try{
		$authorized = checkPerm('POST:/artics/add', $app);
		if($authorized){
				$resultText = checkToken($app);
				if(contains("validtoken", $resultText) ){
					$tableName = 'articulos';
		      ...
		      $connection = null;
				...
				$connection = null;
				$app->response->body("/artics/add (POST) " . ACCESSERROR);
...
SQLS: 	 autogenerado
--*/
$app->put('/artics/delete', function () use($app) {

	try{
		$authorized = checkPerm('PUT:/artics/delete', $app);
		if($authorized){
					$resultText = checkToken($app);
					if(contains("validtoken", $resultText) ){
						$tableName = 'articulos';
				...
				$connection = null;
				$app->response->body("/artics/delete (PUT) " . ACCESSERROR);

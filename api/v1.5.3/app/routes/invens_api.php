...
SQLS: 	 [tabla]_count
--*/
$app->get('/invens/count', function () use($app) {

	try{
			$authorized = checkPerm('GET:/invens/count', $app);
			if($authorized){
				$resultText = checkToken($app);
				if(contains("validtoken", $resultText) ){
		      $sqlCode = 'invens_count';
		      ...
		      simpleReturn($app, $sqlCode, $forXSL);
				...
				$connection = null;
				$app->response->body("/invens/count " . ACCESSERROR);
...
SQLS: 	 [tabla]_all
--*/
$app->get("/invens", function() use($app)
{
 	try{
			$authorized = checkPerm('GET:/invens', $app);
			if($authorized){
					$resultText = checkToken($app);
					if(contains("validtoken", $resultText) ){
						$sqlCode = ($app->request()->params('sqlCode') == "" ? 'invens_all' : $app->request()->params('sqlCode') );
			      $forXSL = '../../xsl/count.xsl';
			      simpleReturn($app, $sqlCode, $forXSL);
					} else {
						$connection = null;
					...
					$connection = null;
					$app->response->body("/invens " . ACCESSERROR);
		}
	}
	catch(PDOException $e)
	{
...
--*/
$app->get("/invens/:id", function($id) use($app)
{
 	try{
		$authorized = checkPerm('GET:/invens/:id', $app);
		if($authorized){
				$resultText = checkToken($app);
				if(contains("validtoken", $resultText) ){
					$sqlCode = 'invens_all';
			...
			$connection = null;
			$app->response->body("/invens/:id " . ACCESSERROR);
...
SQLS: 	 [tabla]_add
--*/
$app->post('/invens', function () use($app) {

	try{
		$authorized = checkPerm('POST:/invens', $app);
		if($authorized){
					$resultText = checkToken($app);
					if(contains("validtoken", $resultText) ){
/*
...
*/
							...
							$prepParams = array(
										...
										':id'         		=> $newId,
										':lugar'  				=> $app->request()->params('lugar'),
										':idart'      		=> $app->request()->params('idart'),
										':unidad'    			=> $app->request()->params('unidad'),
										':cantidad'   		=> $app->request()->params('cantidad'),
										':fechaant'    		=> $app->request()->params('fechaant'),
										':cantant'    		=> $app->request()->params('cantant'),
										':fechasig'    		=> $app->request()->params('fechasig'),
										':observaciones' 	=> $app->request()->params('observaciones'),
		...
		*/
				...
				$connection = null;
				$app->response->body("/invens (POST) " . ACCESSERROR);
...
SQLS: 	 autogenerado
--*/
$app->put('/invens', function () use($app) {

	try{
		$authorized = checkPerm('PUT:/invens', $app);
		if($authorized){
				$resultText = checkToken($app);
				if(contains("validtoken", $resultText) ){
		      ...
		      $connection = null;
				...
				$connection = null;
				$app->response->body("/invens (PUT) " . ACCESSERROR);
...
MÉTODO: POST
REQUERIMIENTOS: TO-DO identificar el req actualización de usuarios
TESTS: api/invens_update_post.sh
...
SQLS: 	 autogenerado
--*/
$app->post('/invens/update', function () use($app) {

	try{
		$authorized = checkPerm('POST:/invens/update', $app);
		if($authorized){
				$resultText = checkToken($app);
				if(contains("validtoken", $resultText) ){
					$tableName = 'inventarios';
		      ...
		      $connection = null;
				...
				$connection = null;
				$app->response->body("/invens/update (POST) " . ACCESSERROR);
...
SQLS: 	 autogenerado
--*/
$app->post('/invens/add', function () use($app) {

	try{
		$authorized = checkPerm('POST:/invens/add', $app);
		if($authorized){
				$resultText = checkToken($app);
				if(contains("validtoken", $resultText) ){
					$tableName = 'inventarios';
		      ...
		      $connection = null;
				...
				$connection = null;
				$app->response->body("/invens/add (POST) " . ACCESSERROR);
...
SQLS: 	 autogenerado
--*/
$app->put('/invens/delete', function () use($app) {

	try{
		$authorized = checkPerm('PUT:/invens/delete', $app);
		if($authorized){
					$resultText = checkToken($app);
					if(contains("validtoken", $resultText) ){
						$tableName = 'inventarios';
				...
				$connection = null;
				$app->response->body("/invens/delete (PUT) " . ACCESSERROR);

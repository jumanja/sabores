...
SQLS: 	 [tabla]_count
--*/
$app->get('/places/count', function () use($app) {

	try{
			$authorized = checkPerm('GET:/places/count', $app);
			if($authorized){
				$resultText = checkToken($app);
				if(contains("validtoken", $resultText) ){
		      $sqlCode = 'places_count';
		      ...
		      simpleReturn($app, $sqlCode, $forXSL);
				...
				$connection = null;
				$app->response->body("/places/count " . ACCESSERROR);
...
{
 	try{
			$authorized = checkPerm('GET:/places', $app);
			if($authorized){
					$resultText = checkToken($app);
					if(contains("validtoken", $resultText) ){
					...
					$connection = null;
					$app->response->body("/places " . ACCESSERROR);
		}
	}
	catch(PDOException $e)
	{
...
{
 	try{
		$authorized = checkPerm('GET:/places/:id', $app);
		if($authorized){
				$resultText = checkToken($app);
				if(contains("validtoken", $resultText) ){
					$sqlCode = 'places_all';
			...
			$connection = null;
			$app->response->body("/places/:id " . ACCESSERROR);
...
$app->post('/places', function () use($app) {
	...
	try{
		$authorized = checkPerm('POST:/places', $app);
		if($authorized){
					$resultText = checkToken($app);
					if(contains("validtoken", $resultText) ){
/*
...
*/
		...
		*/
				...
				$connection = null;
				$app->response->body("/places (POST) " . ACCESSERROR);
...
TESTS: api/places_update.sh

DESCRIPCIÓN: Actualiza los datos de un usuario en la base de datos.

ENTRADA: Token y el Id del usuario de la sesión, y el id del usuario a actualizar,
...
$app->put('/places', function () use($app) {
	...
	try{
		$authorized = checkPerm('PUT:/places', $app);
		if($authorized){
				$resultText = checkToken($app);
				if(contains("validtoken", $resultText) ){
					$tableName = 'lugares';
		      ...
		      $connection = null;
				...
				$connection = null;
				$app->response->body("/places (PUT) " . ACCESSERROR);
...
MÉTODO: POST
REQUERIMIENTOS: TO-DO identificar el req actualización de usuarios
TESTS: api/places_update_post.sh
...
SQLS: 	 autogenerado
--*/
$app->post('/places/update', function () use($app) {

	try{
		$authorized = checkPerm('POST:/places/update', $app);
		if($authorized){
				$resultText = checkToken($app);
				if(contains("validtoken", $resultText) ){
					$tableName = 'lugares';
		      ...
		      $connection = null;
				...
				$connection = null;
				$app->response->body("/places/update (POST) " . ACCESSERROR);
...
SQLS: 	 autogenerado
--*/
$app->put('/places/delete', function () use($app) {

	try{
		$authorized = checkPerm('PUT:/places/delete', $app);
		if($authorized){
					$resultText = checkToken($app);
					if(contains("validtoken", $resultText) ){
						$tableName = 'lugares';
				...
				$connection = null;
				$app->response->body("/places/delete (PUT) " . ACCESSERROR);

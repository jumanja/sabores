...
SQLS: 	 [tabla]_count
--*/
$app->get('/recipes/count', function () use($app) {

	try{
			$authorized = checkPerm('GET:/recipes/count', $app);
			if($authorized){
				$resultText = checkToken($app);
				if(contains("validtoken", $resultText) ){
		      $sqlCode = 'recipes_count';
		      ...
		      simpleReturn($app, $sqlCode, $forXSL);
				...
				$connection = null;
				$app->response->body("/recipes/count " . ACCESSERROR);
...
{
 	try{
			$authorized = checkPerm('GET:/recipes', $app);
			if($authorized){
					$resultText = checkToken($app);
					if(contains("validtoken", $resultText) ){
						$sqlCode = ($app->request()->params('sqlCode') == "" ? 'recipes_all' : $app->request()->params('sqlCode') );
			      $forXSL = '../../xsl/count.xsl';
			      simpleReturn($app, $sqlCode, $forXSL);
					} else {
						$connection = null;
					...
					$connection = null;
					$app->response->body("/recipes " . ACCESSERROR);
		}
	}
	catch(PDOException $e)
	{
...
{
 	try{
		$authorized = checkPerm('GET:/recipes/:id', $app);
		if($authorized){
				$resultText = checkToken($app);
				if(contains("validtoken", $resultText) ){
					$sqlCode = 'recipes_all';
			...
			$connection = null;
			$app->response->body("/recipes/:id " . ACCESSERROR);
...
$app->post('/recipes', function () use($app) {
	...
	try{
		$authorized = checkPerm('POST:/recipes', $app);
		if($authorized){
					$resultText = checkToken($app);
					if(contains("validtoken", $resultText) ){
/*
...
*/
							...
							$prepParams = array(
										...
										':preparacion' 		=> mysql_real_escape_string($app->request()->params('preparacion')),
		...
		*/
				...
				$connection = null;
				$app->response->body("/recipes (POST) " . ACCESSERROR);
...
SQLS: 	 autogenerado
--*/
$app->put('/recipes', function () use($app) {

	try{
		$authorized = checkPerm('PUT:/recipes', $app);
		if($authorized){
				$resultText = checkToken($app);
				if(contains("validtoken", $resultText) ){
					$tableName = 'recetas';
		      ...
		      $connection = null;
				...
				$connection = null;
				$app->response->body("/recipes (PUT) " . ACCESSERROR);
...
MÉTODO: POST
REQUERIMIENTOS: TO-DO identificar el req actualización de usuarios
TESTS: api/recipes_update_post.sh
...
SQLS: 	 autogenerado
--*/
$app->post('/recipes/update', function () use($app) {

	try{
		$authorized = checkPerm('POST:/recipes/update', $app);
		if($authorized){
				$resultText = checkToken($app);
				if(contains("validtoken", $resultText) ){
					$tableName = 'recetas';
		      ...
		      $connection = null;
				...
				$connection = null;
				$app->response->body("/recipes/update (POST) " . ACCESSERROR);
...
SQLS: 	 autogenerado
--*/
$app->post('/recipes/add', function () use($app) {

	try{
		$authorized = checkPerm('POST:/recipes/add', $app);
		if($authorized){
				$resultText = checkToken($app);
				if(contains("validtoken", $resultText) ){
					$tableName = 'recetas';
		      ...
		      $connection = null;
				...
				$connection = null;
				$app->response->body("/recipes/add (POST) " . ACCESSERROR);
...
SQLS: 	 autogenerado
--*/
$app->put('/recipes/delete', function () use($app) {

	try{
		$authorized = checkPerm('PUT:/recipes/delete', $app);
		if($authorized){
					$resultText = checkToken($app);
					if(contains("validtoken", $resultText) ){
						$tableName = 'recetas';
				...
				$connection = null;
				$app->response->body("/recipes/delete (PUT) " . ACCESSERROR);

...
SQLS: 	 [tabla]_count
--*/
$app->get('/tags/count', function () use($app) {

	try{
			$authorized = checkPerm('GET:/tags/count', $app);
			if($authorized){
		      ...
		      simpleReturn($app, $sqlCode, $forXSL);
				...
				$connection = null;
				$app->response->body("/tags/count " . ACCESSERROR);
...
{
 	try{
			$authorized = checkPerm('GET:/tags', $app);
			if($authorized){
					$resultText = checkToken($app);
					if(contains("validtoken", $resultText) ){
					...
					$connection = null;
					$app->response->body("/tags " . ACCESSERROR);
		}
	}
	catch(PDOException $e)
	{
...
{
 	try{
		$authorized = checkPerm('GET:/tags/:id', $app);
		if($authorized){
				$resultText = checkToken($app);
				if(contains("validtoken", $resultText) ){
					$sqlCode = 'tags_all';
			...
			$connection = null;
			$app->response->body("/tags/:id " . ACCESSERROR);
...
$app->post('/tags', function () use($app) {
	...
	try{
		$authorized = checkPerm('POST:/tags', $app);
		if($authorized){
					$resultText = checkToken($app);
					if(contains("validtoken", $resultText) ){
/*
...
*/
							...
							$prepParams = array(
										...
										':etiqueta'   => $app->request()->params('etiqueta'),
										':id'         => $newId,
		...
		*/
				...
				$connection = null;
				$app->response->body("/tags (POST) " . ACCESSERROR);
...
SQLS: 	 autogenerado
--*/
$app->put('/tags', function () use($app) {

	try{
		$authorized = checkPerm('PUT:/tags', $app);
		if($authorized){
				$resultText = checkToken($app);
				if(contains("validtoken", $resultText) ){
					$tableName = 'etiquetas';
		      ...
		      $connection = null;
				...
				$connection = null;
				$app->response->body("/tags (PUT) " . ACCESSERROR);
...
SQLS: 	 autogenerado
--*/
$app->post('/tags/update', function () use($app) {

	try{
		$authorized = checkPerm('POST:/tags/update', $app);
		if($authorized){
				$resultText = checkToken($app);
				if(contains("validtoken", $resultText) ){
					$tableName = 'etiquetas';
		      ...
		      $connection = null;
				...
				$connection = null;
				$app->response->body("/tags/update (POST) " . ACCESSERROR);
...
$app->post('/tags/add', function () use($app) {
	...
	try{
		$authorized = checkPerm('POST:/tags/add', $app);
		if($authorized){
				$resultText = checkToken($app);
				if(contains("validtoken", $resultText) ){
					$tableName = 'etiquetas';
		      ...
		      $connection = null;
				...
				$connection = null;
				$app->response->body("/tags/add (POST) " . ACCESSERROR);
...
SQLS: 	 autogenerado
--*/
$app->put('/tags/delete', function () use($app) {

	try{
		$authorized = checkPerm('PUT:/tags/delete', $app);
		if($authorized){
					$resultText = checkToken($app);
					if(contains("validtoken", $resultText) ){
						$tableName = 'etiquetas';
				...
				$connection = null;
				$app->response->body("/tags/delete (PUT) " . ACCESSERROR);

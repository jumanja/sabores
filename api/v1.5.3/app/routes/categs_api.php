...
SQLS: 	 [tabla]_count
--*/
$app->get('/categs/count', function () use($app) {

	try{
			$authorized = checkPerm('GET:/categs/count', $app);
			if($authorized){
				$resultText = checkToken($app);
				if(contains("validtoken", $resultText) ){
		      $sqlCode = 'categs_count';
		      ...
		      simpleReturn($app, $sqlCode, $forXSL);
				...
				$connection = null;
				$app->response->body("/categs/count " . ACCESSERROR);
...
SQLS: 	 [tabla]_all
--*/
$app->get("/categs", function() use($app)
{
 	try{
			$authorized = checkPerm('GET:/categs', $app);
			if($authorized){
					$resultText = checkToken($app);
					if(contains("validtoken", $resultText) ){
						$sqlCode = ($app->request()->params('sqlCode') == "" ? 'categs_all' : $app->request()->params('sqlCode') );
			      $forXSL = '../../xsl/count.xsl';
			      simpleReturn($app, $sqlCode, $forXSL);
					} else {
						$connection = null;
					...
					$connection = null;
					$app->response->body("/categs " . ACCESSERROR);
		}
	}
	catch(PDOException $e)
	{
...
--*/
$app->get("/categs/:id", function($id) use($app)
{
 	try{
		$authorized = checkPerm('GET:/categs/:id', $app);
		if($authorized){
				$resultText = checkToken($app);
				if(contains("validtoken", $resultText) ){
					$sqlCode = 'categs_all';
			...
			$connection = null;
			$app->response->body("/categs/:id " . ACCESSERROR);
...
SQLS: 	 [tabla]_add
--*/
$app->post('/categs', function () use($app) {

	try{
		$authorized = checkPerm('POST:/categs', $app);
		if($authorized){
					$resultText = checkToken($app);
					if(contains("validtoken", $resultText) ){
/*
...
*/
		...
		*/
				...
				$connection = null;
				$app->response->body("/categs (POST) " . ACCESSERROR);
...
SQLS: 	 autogenerado
--*/
$app->put('/categs', function () use($app) {

	try{
		$authorized = checkPerm('PUT:/categs', $app);
		if($authorized){
				$resultText = checkToken($app);
				if(contains("validtoken", $resultText) ){
		      ...
		      $connection = null;
				...
				$connection = null;
				$app->response->body("/categs (PUT) " . ACCESSERROR);
...
MÉTODO: POST
REQUERIMIENTOS: TO-DO identificar el req actualización de usuarios
TESTS: api/categs_update_post.sh
...
SQLS: 	 autogenerado
--*/
$app->post('/categs/update', function () use($app) {

	try{
		$authorized = checkPerm('POST:/categs/update', $app);
		if($authorized){
				$resultText = checkToken($app);
				if(contains("validtoken", $resultText) ){
					$tableName = 'categorias';
		      ...
		      $connection = null;
				...
				$connection = null;
				$app->response->body("/categs/update (POST) " . ACCESSERROR);
...
SQLS: 	 autogenerado
--*/
$app->post('/categs/add', function () use($app) {

	try{
		$authorized = checkPerm('POST:/categs/add', $app);
		if($authorized){
				$resultText = checkToken($app);
				if(contains("validtoken", $resultText) ){
					$tableName = 'categorias';
		      ...
		      $connection = null;
				...
				$connection = null;
				$app->response->body("/categs/add (POST) " . ACCESSERROR);
...
SQLS: 	 autogenerado
--*/
$app->put('/categs/delete', function () use($app) {

	try{
		$authorized = checkPerm('PUT:/categs/delete', $app);
		if($authorized){
					$resultText = checkToken($app);
					if(contains("validtoken", $resultText) ){
						$tableName = 'categorias';
				...
				$connection = null;
				$app->response->body("/categs/delete (PUT) " . ACCESSERROR);
